...
farmers_raw <- read.csv("https://raw.githubusercontent.com/claire-standley/ECNS-560-Term-Project---Claire-Standley/main/Raw%20Data/Farmers%20Market%20Data.csv") |>
  clean_names() #cleans variable name when loading the dataset in (underscores and such)

#census data (county level)
census_raw <-read_csv("https://raw.githubusercontent.com/claire-standley/ECNS-560-Term-Project---Claire-Standley/main/Raw%20Data/USDA%20Census%20Dataset%202002-2022.csv") |>
  clean_names()

#agritourism law data set (state level)
laws_raw <- read_csv("https://raw.githubusercontent.com/claire-standley/ECNS-560-Term-Project---Claire-Standley/main/Raw%20Data/Agritourism%20Law%20Implimentation.csv") |>
  clean_names()
...
census_clean <- census_raw |>
  mutate(
    ...
    commodity_totals_operations_with_sales = 
      parse_number(str_remove_all(commodity_totals_operations_with_sales, "\\(.*?\\)")),
     
    commodity_totals_sales_measured_in = 
      parse_number(str_remove_all(commodity_totals_sales_measured_in, "\\(.*?\\)")),
    
    farms_operations = `commodity_totals_operations_with_sales`,
    farms_income = `commodity_totals_sales_measured_in`
             ) |>
